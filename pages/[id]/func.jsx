...
const MoviePage = ({ success, error, movie }) => {
  ...
  const deleteData = async (id) => {
    try {
      await fetch(`/api/funcionario/${id}`, {
        method: "DELETE",
      });
      router.push("/func");
    } catch (error) {
    ...
    <div className="container">
      ...
      <div className="card">
        <div className="card-body">
          ...
          <p className="fw-light"><b>Data de Nascimento:</b> {movie.dataNascimento}</p>
          <p className="fw-light"><b>Salario:</b> {movie.salario}R$</p>
          <p className="fw-light"><b>Telefone:</b> {movie.telefone}</p>
          <p className="fw-light"><b>Função:</b> {movie.funcao}</p>
          
          <Link href="/func">
            <a className="btn btn-success btn-sm me-2">Voltar</a>
          </Link>
          <Link href={`/${movie._id}/editFunc`}>
...
export async function getServerSideProps({ params }) {
  try {
    ...
    const movie = await Funcionario.findById(params.id).lean();
